...
import { findPois } from '../src/db';
import { GeoBounds } from '../src/types/coordinate';
...
async function loopEndDelete(theresholdSize: number) {
    const mongoUrl = "mongodb+srv://root:root@autotripguide.swdtr.mongodb.net/myFirstDatabase?retryWrites=true&w=majority"
    const db = new AutoTripGuideDB(mongoUrl, 'auto_trip_guide_db', 'cachedAreas');
    ...
    for (const geohash of allGeohashes) {
        ...
        const MAX_POIS_TO_FIND = 20;
        // Find data by params
        const results = await findPois(client, queryObject, relevantBounds, MAX_POIS_TO_FIND, false, geohash);

        // If no POIs found for this geohash, delete it
        if (!results || results.length <= theresholdSize) {
